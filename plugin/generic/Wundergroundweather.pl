...
my $city = "Leipzig";        # Meine Stadt, hier statt &uuml;,&auml;,&ouml; einfach u,a,o nehmen oder ue,ae,oe
...
my $symbolebg                                        = "/symbolebg/";                    # Pfad zu den Wetterhintergr&uuml;nden
...
open(XML, $xml_w);    # XML Datei zum Schreiben &ouml;ffnen
print XML $xml;
close(XML);
...
"<h2 align=center>".$weather->{current_observation}->{temp_c}."&deg;</h2><h3><br/>\n".
"Gef&uuml;hlt: ".$weather->{current_observation}->{feelslike_c}."&deg; C<br/>\n".
...
$weather->{forecast}->{simpleforecast}->{forecastdays}->{forecastday}->[$j]->{high}->{celsius}."&deg; C</font><br/>\n".
$weather->{forecast}->{simpleforecast}->{forecastdays}->{forecastday}->[$j]->{low}->{celsius}."&deg; C</h1><h3><br/>\n".
...
open(HTML, ">:utf8", $html_datei);    # HTML Datei zum Schreiben &ouml;ffnen
print HTML $html;
close(HTML);
